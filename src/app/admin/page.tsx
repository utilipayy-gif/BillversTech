...
export default async function AdminPage({ searchParams }: PageProps<"/admin">) {
  ...
  if (!(await isAdminAuthenticated())) {
    return <main className={`${styles.shell} ${feedback.validationScope}`}>
      <section className={styles.login}>
        <span>BILLVERSTECH / CONTENT ADMIN</span>
  ...
  return <main className={`${styles.shell} ${feedback.validationScope}`}>
    <header className={styles.header}>
      <div><span>BILLVERSTECH / ADMIN</span><h1>Services</h1><p>{services.length} live service pages</p></div>
      <div>
    ...
    <details className={styles.create}>
      ...
      <form action={updateSettingsAction}>
        <div className={styles.pair}>
          ...
          <label>WhatsApp number (country code, digits only)<input name="whatsapp" defaultValue={settings.whatsapp} pattern="[0-9]{10,15}" title="Enter 10 to 15 digits including the country code." required /></label>
        </div>
    ...
    <section className={styles.list}>
      {services.map((service) => <details className={styles.card} key={service.slug}>
        <summary>
          <div><span>{service.category}</span><h2>{service.title}</h2><p>/services/{service.slug}</p></div>
          <b>Edit ↓</b>
        </summary>
